...
use App\Jobs\Notifications\SendBulkPushNotification;
...
trait FetchDriversFromFirebaseHelpers
{
    ...
    {
        // \Log::info('maximum_time_for_find_drivers_for_regular_ride '. get_settings('maximum_time_for_find_drivers_for_regular_ride'));
        $url = url()->current();   // e.g. https://example.com/users
        ...
        //      return null;   
        // }
                    // \Log::info('debug-push start', [
                    //         'url' => $url,
                    //         'urlWithQuery' => $urlWithQuery,
                    //         'trace' => debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS, 5) // 5 level only
        ...
        if($only_luxury_limoexpress == 1){
            ...
            if($only_luxury_limoexpress == 2){
                ...
                if(!empty($NotIcludeOwners)){
                    ...
                    \Log::info('ifride', [$Owners]);
                } else {
                    
                    $Owners = Owner::get();
                    \Log::info('elseride', [$Owners]);
                }
            }
            foreach($Owners as $Owner){
                $cacheKey = 'sent_request_owners_' . $request_detail->id;
                $sentOwners = Cache::get($cacheKey, []);
                if (in_array($Owner->id, $sentOwners)) {
                    Log::info("Skipping owner {$Owner->id} — already got request {$request_detail->id}");
                    continue;
                }
                ...
                $push_data = ['title' => $title,'message' => $body,'push_type'=>'meta-request'];
                try {
                    // \Log::info('debug-push 2');
                    dispatch(new SendPushNotification($notifable_owner,$title,$body,$push_data));
                    $sentOwners[] = $Owner->id;
                    Cache::put($cacheKey, $sentOwners, now()->addHours(2));
                } catch (\Exception $e) {
        ...
        $current_date = Carbon::now();
         ...
         if (!empty($firebase_drivers)) {
            ...
            // if(count($nearest_driver_ids)==0){
                ...
                $notifiable_drivers = [];
                $i = 0;
                Log::info('fetchDriversFromFirebaseHelpers nearest_drivers count '. $nearest_drivers->count(), [$nearest_drivers]);
                foreach ($nearest_drivers as $driver_key => $driver) {
                    $cacheKey = 'sent_request_drivers_' . $request_detail->id;
                    $sentDrivers = Cache::get($cacheKey, []);

                    // 2. Skip if driver already got this request
                    if (in_array($driver->id, $sentDrivers)) {
                        Log::info("Skipping driver {$driver->id} — already got request {$request_detail->id}");
                        continue;
                    }
                    ...
                    // $push_data = ['title' => $title,'message' => $body,'push_type'=>'meta-request', "priority" => "high"];

                    try {
                        \Log::info('debug-push driver name:  ', [$notifable_driver->name]);
                        // dispatch(new SendPushNotification($notifable_driver,$title,$body,$push_data));
                        $notifiable_drivers[] = $notifable_driver;
                        $sentDrivers[] = $driver->id;
                        Cache::put($cacheKey, $sentDrivers, now()->addHours(2)); // keep for 2 hours

                    } catch (\Exception $e) {
                        \Log::info('debug-push error 9', ['error' => $e->getMessage()]);
                ...
                try {
                    \Log::info('Start SendBulkPushNotification', [
                        'time' => now()->toDateTimeString(),
                        $notifiable_drivers
                    ]);
                    if (!empty($notifiable_drivers) && count($notifiable_drivers) > 0) {
                        dispatch(new SendBulkPushNotification($notifiable_drivers, 'push_notifications.new_request_title', 'push_notifications.new_request_body', ['push_type'=>'meta-request', "priority" => "high"]));
                    }
                    \Log::info('End SendBulkPushNotification', [
                        'time' => now()->toDateTimeString()
                    ]);
                } catch (\Exception $e) {
