...
class DispatcherController extends BaseController
{
    ...
    */
    ...
    public function book_ride(Request $request)
    {
        ...
        // 👇 Add this line to get all active drivers
    ...
    public function store(Request $request)
    {
        $request->validate([
            'radiobtn' => 'required',
            'pickup_addr' => 'required|string',
            'drop_addr' => 'required|string',
            'pickup_lat' => 'required',
            'pickup_lng' => 'required',
            'own_price' => 'required|numeric',
            'comission_percentage' => 'nullable|numeric',
            'manual_date' => 'nullable|date',
            'manual_hour' => 'nullable|numeric',
            'manual_ref_name' => 'nullable|string',
        ]);

        $ride = new Driver(); // ⚠ You may want to use a Ride or Booking model instead

        $ride->pickup_address = $request->pickup_addr;
        $ride->drop_address = $request->drop_addr;
        $ride->pickup_lat = $request->pickup_lat;
        $ride->pickup_lng = $request->pickup_lng;
        $ride->own_price = $request->own_price;
        $ride->commission = $request->comission_percentage ?? 0;

        // Handle manual assignment
        if ($request->radiobtn == 1) {
            $ride->is_manual = true;
            $ride->assigned_driver_id = $request->input('assigned_driver_id');
            $ride->manual_ref_name = $request->manual_ref_name;
            $ride->manual_amount = $request->manual_amount;
            $ride->manual_commission = $request->manual_commission;

            if ($request->manual_date && $request->manual_hour) {
                $ride->scheduled_at = Carbon::parse($request->manual_date)->addHours($request->manual_hour);
            }
        } else {
            // Auto assignment
            $ride->is_manual = false;
            $excludedDrivers = $request->input('not_include_owner', []);
            $ride->excluded_drivers = json_encode($excludedDrivers);
        }
    ...
    {
        $ride = Driver::findOrFail($id);

        if ($ride->status === 'completed' || $ride->is_manual) {
            $ride->status = 'cancelled';
            $ride->cancelled_by = Auth::id();
            $ride->cancelled_at = now();
            $ride->assigned_driver_id = null;

            $ride->save();

            // Optionally also remove from Firebase
            // $this->database->getReference('request-meta/' . $id)->remove();
        }
