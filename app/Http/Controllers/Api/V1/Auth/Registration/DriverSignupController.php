...
if ($request->has('mobile') && $request->has('email')) {
    ...
    */
    ...
    public function ownerRegister(Request $request){
            Log::info('ownerRegister : ' . json_encode($request->all()));
    ...
    {
            Log::error('updatePassword ' . json_encode($request->all()));
...
/*mobile Ends*/
    ...
    public function switchLogin(Request $request)
    {
        ...
        $roleName = $request->role === 'driver' ? Role::DRIVER : Role::OWNER;
        ...
        if (!$user) {
            return $this->respondBadRequest('User not found or invalid role');
        }
