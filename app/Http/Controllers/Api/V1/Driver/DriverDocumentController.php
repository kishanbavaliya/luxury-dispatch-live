...
class DriverDocumentController extends BaseController
{
    ...
    */
    ...
    {   
        ...
        foreach ($driver_needed_docs as $key => $needed_document) {
            ...
            // If owner requests needed documents for a specific driver
            if (auth()->user()->hasRole(Role::OWNER) && $request->has('driver_id') && $request->driver_id) {
                $driver = Driver::where('id', $request->driver_id)->first();
                if (! $driver) {
                    return response()->json(['success' => false, 'message' => 'Driver not found'], 404);
                }

                // Ensure the owner owns this driver
                $owner_id = auth()->user()->owner->id;
                if ($driver->owner_id != $owner_id) {
                    return response()->json(['success' => false, 'message' => 'Not authorized for this driver'], 403);
                }
                ...
                foreach ($driver_needed_docs as $key => $needed_document) {
                    if (DriverDocument::where('driver_id', $driver->id)->where('document_id', $needed_document->id)->exists()) {
                        $uploaded_document = true;
                    } else {
                        $uploaded_document = false;
                    }
    ...
    */
    public function uploadDocuments(DriverDocumentUploadRequest $request)
    {
        ...
        if ($fleet_documents) {
        ...
        } elseif ($request->has('driver_id') && $request->driver_id && auth()->user()->hasRole(Role::OWNER)) {

            // Owner uploading documents for a specific driver
            $driver = Driver::where('id', $request->driver_id)->first();
            if (! $driver) {
                return response()->json(['success' => false, 'message' => 'Driver not found'], 404);
            }
            ...
            $document_exists = DriverDocument::where('driver_id', $driver->id)->where('document_id', $request->document_id)->exists();
            ...
            // Check if document exists
            $driver_documents = DriverDocument::where('driver_id', $driver->id)->where('document_id', $request->input('document_id'))->first();
            // dd($created_params);
            if ($driver_documents) {
                DriverDocument::where('driver_id', $driver->id)->where('document_id', $request->input('document_id'))->update($created_params);
            } else {
                DriverDocument::create($created_params);
            }
